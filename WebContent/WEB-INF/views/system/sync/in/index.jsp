...
<div id="list_container">
	<table class="repeattable">
		<caption>
			...
			<a class="button_sync" href="javascript:void(0);" onclick="syncData('LicenseApp');">同步许可申请</a>
			<%-- </sec:authorize> --%>
			<%-- <sec:authorize access="hasRole('OA_ROLE_DELETE')"> --%>
			<a class="button_sync" href="javascript:void(0);" onclick="syncData('Complaints');">同步投诉举报</a>
			<%-- </sec:authorize> --%>
			<%-- <sec:authorize access="hasRole('OA_ROLE_DELETE')"> --%>
			<a class="button_sync" href="javascript:void(0);" onclick="resyncData();">重发</a>
			<%-- </sec:authorize> --%>
			<%-- <sec:authorize access="hasRole('OA_ROLE_DELETE')"> --%>
			<a class="button_del_all" href="javascript:void(0);" onclick="helper.deleteObjects(null,'/proxy/CMS/sync/in/delete');">删除</a>
			<%-- </sec:authorize> --%>
		</caption>
		<colgroup>
		...
		<tr id="template" style="display: none;">
			...
			<td align="center">
				<div class="option_container">
					...
					<%-- </sec:authorize> --%>
					<%-- <sec:authorize access="hasRole('OA_ROLE_DELETE')"> --%>
					<a href="javascript:void(0);" title="删除" class="option_delete" onclick="helper.deleteObjects('#id#','/proxy/CMS/sync/in/delete');"></a>
					<%-- </sec:authorize> --%>
				</div>
			</td>
		</tr>
...
var syncData = function(type) {
	top.Dialog.confirm("你确定要同步吗？", function() {
		helper.basicPost("/proxy/CMS/sync/in/syncWithType", { type : type }, 3, null, function(params) {
			helper.basicPost("/sync", { params : params }, 3, null,  function(data) {
				helper.basicPost("/proxy/CMS/sync/in/updateFlag", { params : params }, 3, null,  function(response) {
...
var search = function() {
	helper.postSearch('/proxy/CMS/sync/in/search', {
		formId : 'search_form',
...
var view = function(id) {
	helper.basicPost('/proxy/CMS/sync/in/view', {
		'id' : id
	}, 2, {
...
var resyncData = function(id) {
	//helper.operateObjects(id, "/proxy/CMS/sync/in/reSync", "重发");
	var idList = "";
	if (!isNull(id) && id != "") {
		// operate one
		idList = id;
	...
	top.Dialog.confirm("你确定要重发吗？", function() {
		helper.basicPost("/proxy/CMS/sync/in/reSync", { id : idList }, 3, null, function(params) {
			helper.basicPost("/sync", { params : params }, 3, null, function(data) {
				helper.basicPost("/proxy/CMS/sync/in/updateFlag", { params : params }, 3, null,  function(response) {
					top.Dialog.alert("同步成功");
				});
