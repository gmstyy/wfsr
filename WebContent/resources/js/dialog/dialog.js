var IMAGESPATH = ctx+'/images/dialog/'; //图片路径配置
/*************************一些公用方法和属性****************************/
...
Dialog.prototype.displacePath = function () {
	...
	*/
	// TIPS: MODIFY THIS CODE FOR SYSTEM INTEGRATION, FOUNDER DO NOT USE THE URL METHOD
	if (this.URL.substr(0, 7) == "http://" || this.URL.substr(0, 11) == "javascript:") {
        return this.URL;
    }
	return getContextUrl("/proxy/ZSY/" + this.URL);
};
...
Dialog.prototype.create = function () {
  ...
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0"  style="font-size:12px; line-height:1.4;">\
	...
	' + (function (obj) {
        ...
        if (obj.URL) {
        	return '<iframe width="100%" height="100%" frameborder="0" style="border:none 0;" allowtransparency="true" id="_DialogFrame_' + obj.ID + '" src="' + obj.displacePath() + '"></iframe>';
        }
        return "";
    })(this) + '\
              </div></td>\
          ...
          <tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_' + this.ID + '" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: right; background-color:#f6f6f6;">\
                <input type="button" class="buttonStyle" value="' + this.okButtonText + '" id="_ButtonOK_' + this.ID + '"/>\
                <input type="button" class="buttonStyle" value="' + this.cancelButtonText + '" onclick="Dialog.getInstance(\'' + this.ID + '\').close();" id="_ButtonCancel_' + this.ID + '"/>\
              </div></td>\
          </tr>\
        </table></td>\
...
// 自定义打开对话框方法
Dialog.open = function(containerId, title, width, height, userCallbacks, initFun) {
	var dialog = new Dialog();
	dialog.InvokeElementId = containerId;
	dialog.Title = title;
	dialog.Width = width;
	dialog.Height = height;
	var okEnabled = true, cancelEnabled = true;
	if (typeof(userCallbacks) != "undefined") {
		dialog.ShowButtonRow = true;
		if (typeof(userCallbacks.defaultButtons) != "undefined") {
			if (typeof(userCallbacks.defaultButtons.okEnabled) == "undefined" || userCallbacks.defaultButtons.okEnabled == true) {
				if (typeof(userCallbacks.okCallback) == "function") {
					dialog.OKEvent = userCallbacks.okCallback;
				}
				if (typeof(userCallbacks.defaultButtons.okText) != "undefined") {
					dialog.okButtonText = userCallbacks.defaultButtons.okText;
				}
			} else {
				okEnabled = false;
			}
			if (typeof(userCallbacks.defaultButtons.cancelEnabled) == "undefined" || userCallbacks.defaultButtons.cancelEnabled == true) {
				if (typeof(userCallbacks.cancelCallback) == "function") {
					dialog.CancelEvent = userCallbacks.cancelCallback;
				}
				if (typeof(userCallbacks.defaultButtons.cancelText) != "undefined") {
					dialog.cancelButtonText = userCallbacks.defaultButtons.cancelText;
				}
			} else {
				cancelEnabled = false;
			}
	...
	dialog.show();
	if (!okEnabled) {
		$("#_ButtonOK_" + dialog.ID).hide();
	}
	if (!cancelEnabled) {
		$("#_ButtonCancel_" + dialog.ID).hide();
	}
	if (typeof(userCallbacks) != "undefined" && typeof(userCallbacks.buttons) != "undefined") {
		var userButtons = [], currentButton;
		for (var buttonText in userCallbacks.buttons) {
			userButtons.push({ text : buttonText, callback : userCallbacks.buttons[buttonText] });
		}
		for (var index = userButtons.length - 1; index >= 0; index--) {
			currentButton = userButtons[index];
			dialog.addButton(index + 10, currentButton.text, currentButton.callback);
		}
	}
};
Dialog.prototype.setButtonStatus = function(enabled) {
	var buttons = topWin.$id("_DialogButtons_" + this.ID).getElementsByTagName("INPUT");
	for (var cnt = 0; cnt < buttons.length; cnt++) {
		buttons[cnt].disabled = !enabled;
	}
};
Dialog.disableButtons = function () {
	if (topWin.Dialog._Array.length > 0) {
        var diag = topWin.Dialog._Array[topWin.Dialog._Array.length - 1];
    	diag.setButtonStatus(false);
    }
};
Dialog.enableButtons = function () {
	var size = topWin.Dialog._Array.length;
	if (size > 0) {
        var diag = topWin.Dialog._Array[size - 1];
        //if(diag.Title == '系统提示' && size > 1){
        	//diag = topWin.Dialog._Array[size - 2];
        //}
        diag.setButtonStatus(true);
    }
};
